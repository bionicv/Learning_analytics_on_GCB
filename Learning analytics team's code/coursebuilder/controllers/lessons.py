...
class YouTubeHandler(BaseHandler):
    def update_youtube_event_transaction(self, email, video, event, info,date):
        ...
        youtubeEvent=models.YouTubeEvent(user_id=student.user_id)
        youtubeEvent.video=video
        youtubeEvent.event=event
        youtubeEvent.info=info
        youtubeEvent.date=date
    ...
    def post(self):
        "Receives the event and it stores in the data store"
        student = self.personalize_page_and_get_enrolled()
        if not student:
            return
        ...
        # Record data
        self.update_youtube_event_transaction(student.key().name(),video,event,info,date)